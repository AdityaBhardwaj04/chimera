...
const Navbar = () => {
    ...
    const listenScrollEvent = () => {
        window.scrollY > 30 ? setnavColor("backdrop-blur-md") : setnavColor("white");
    };
            ...
            <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto px-4 py-2 z-50">
                <div className="flex title-font font-base items-center text-gray-900 mb-4 md:mb-0">
                    <Link to={'/'} className=" " onClick={() => changecolor("homecolor")}>
                        <img src={chimeraLogo} alt="kritarthlogo" className="logo_shadow  h-16 w-full  backdrop-sha" />
                    </Link>
                </div>
                <div className="flex md:order-2">
                    <div type="button" className="mr-3 md:mr-0 flex justify-center items-center space-x-2">
                        <a href="https://ksac.kiit.ac.in/" target="_blank" rel="noreferrer">
                            <img src={ksacLogo} alt="ksaclogo" className="h-20 img p-1" />
                        </a>
                        <a href="https://kiit.ac.in/" target="_blank" rel="noreferrer">
                            <img src={kiitLogo} alt="kiitlogo" className="h-16 img p-1" />
                        </a>
                    </div>
                </div>
                <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-cta">
                    <div className="flex justify-center items-center space-x-4  text-black font-semibold nav_font">
                        <Link onClick={() => { changecolor("about") }} to={"/about"} className={`md:font-semiblod ${about && 'bg-[#3CA465]'} px-3 py-1   md:rounded-xl hover:text-gray-300`}>
                            About
                        </Link>
                        <Link onClick={() => { changecolor("evants") }} to={"/event"} className={`md:font-semiblod ${event && 'bg-[#3CA465]'} px-3 py-1   md:rounded-xl hover:text-gray-300`}>
                            Events
                        </Link>
                        <Link onClick={() => { changecolor("leads") }} to={"/leads"} className={`md:font-semiblod ${leads && 'bg-[#3CA465]'} px-3 py-1   md:rounded-xl hover:text-gray-300`}>
                            Leads
                        </Link>
                        <Link onClick={() => { changecolor("reg") }} to={"/commingsoon"} className={`md:font-semiblod ${reg && 'bg-[#3CA465]'} px-3 py-1   md:rounded-xl hover:text-gray-300`}>
                            Register
                        </Link>
                        <Link onClick={() => { changecolor("contact") }} to={"/contacts"} className={`md:font-semiblod ${contact && 'bg-[#3CA465]'} px-3 py-1   md:rounded-xl hover:text-gray-300`}>
                            Contact
                        </Link>
                    </div>
                </div>
            ...
            <div className={`mx-auto ${navColor} ${click ? "backdrop-blur-md" : ""} max-w-7xl px-2 md:px-6 lg:px-8`}>
                <div className="relative flex h-16 items-center justify-between">
                    ...
                    <div className="flex flex-1 items-center justify-center md:items-stretch md:justify-start">
                        <div className="flex flex-shrink-0 items-center">
                            <Link to={'/'}>
                                <img className="h-8 w-auto" src={chimeraLogo} alt="Your Company"/>
                            </Link>
                        </div>
                    </div>
                    <div className="absolute inset-y-0 right-0 flex items-center pr-2 md:static md:inset-auto md:ml-6 md:pr-0">
                        ...
                        <div className="relative ml-3">
                            <div>
                                <div className="relative flex   text-sm ">
                                    <a href="https://ksac.kiit.ac.in/" target="_blank" rel="noreferrer">
                                        <img className="h-11 w-auto p-1" src={ksacLogo} alt="Your Company"/>
                                    </a>
                                    <a href="https://kiit.ac.in/" target="_blank" rel="noreferrer">
                                        <img className="h-10 w-12 py-[2px] p-1" src={kiitLogo} alt=""/>
                                    </a>
                                </div>
                            </div>
                        </div>
